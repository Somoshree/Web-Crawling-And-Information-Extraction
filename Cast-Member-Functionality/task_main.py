	...
	while True:
		...
		if "movie name"==field_name.lower():   #field name to be retrieved is movie name
			...
			if result['cast'] == {}:
				...
				print("Cast members : ")
				for key in result['cast']:
					print(key.strip()+"\n")
					fileptr.write("<"+genre+">"+" <"+actual_moviename+">"+" <"+"Cast and Crew"+">"+" <"+key.strip()+">\n")
				castname=input("Enter the name of a cast member from the above list of members!\n")
				flag=0
				while True:
					for key,value in result['cast'].items():
						if key.split("as")[0].strip().lower()==castname.lower():
							flag=1
							actual_castname=key.split("as")[0].strip()
							try:
								response=urllib.request.urlopen(value)
								webContent=response.read()
								f=open(actual_castname+'.html','wb')
								f.write(webContent)
								print("The corresponding cast memeber's HTML page has been saved successfully!\n")  #saves the html webpage of the corresponding movie entered by the user
								f.close()
							except:
								print("The webpage of "+actual_castname+" is not available. Hence no information can be fetched! Exiting...")
								exit()
							break
					if flag==0:
						castname=input("The cast member's name that you entered wasn't found!Please enter a correct name from the above list or enter 'E' to exit!\n")
						if castname=='E':
							exit()
					else:
						break
				castname=actual_castname+'.html'
				print("Parsing the HTML file...\n")
				cast_details['highest_rated']=''
				cast_details['lowest_rated']=''
				cast_details['birthday']=''
				cast_details['other_movies']=dict()
				with open(castname, 'r') as myfile:
					data = myfile.read()
					lexer = lex.lex()   #lexical analyzer that generates the tokens
					parser = yacc.yacc()  
					parser.parse(data)   #parses the html webpage
				print("Parsing completed!!\n")
				while True:
					cast_field=input("Enter the field name that you wish to query about the cast member "+actual_castname+": \n\
						1. Highest Rated Film \n\
						2. Lowest Rated Film \n\
						3. Birthday \n\
						4. His/Her other movies \n\
						Please enter 'E' if you wish to exit! \n")
					if cast_field=='E':
						break
					if "highest rated film"==cast_field.lower():
						if cast_details['highest_rated']!='':
							print("The highest rated film of "+actual_castname+" is: ",cast_details['highest_rated']+"\n")
						else:
							print("The highest rated film of "+actual_castname+" couldn't be retrieved!\n")
					elif "lowest rated film"==cast_field.lower():
						if cast_details['lowest_rated']!='':
							print("The lowest rated film of "+actual_castname+" is: ",cast_details['lowest_rated']+"\n")
						else:
							print("The lowest rated film of "+actual_castname+" couldn't be retrieved!\n")
					elif "birthday"==cast_field.lower():
						if cast_details['birthday']!='':
							print("The birth date of "+actual_castname+" is: ",cast_details['birthday']+"\n")
						else:
							print("The birth date of "+actual_castname+" couldn't be retrieved!\n")
					elif "his/her other movies"==cast_field.lower():
						print(actual_castname+"'s other movies are listed below: \n")
						for key in cast_details['other_movies']:
							print(key.strip()+"\n")
						year=input("Enter an year on and after which all the movies done by "+actual_castname+" you want to be displayed or enter 'E' to exit!\n")
						while True:
							if year=='E':
								break
							try:
								...
								print("Please enter a valid year or press 'E' to exit!")
								year=input()
								continue
							print("The movies done by "+actual_castname+" on and after the year "+year+" are as follows: \n")
							for key,value in cast_details['other_movies'].items():
								try:
									if value != '' and int(value)>=yr:
										print(key.strip()+"\n")	
								except:
									starting_year=value.split('-')[0]
									if int(starting_year)>=yr:
										print(key.strip()+"\n")
							break
					else:
						cast_field=input("You entered an incorrect field name!!\n")
			...
			if result['youmaylike'] != {}:
				...
				print("The field 'You May Also Like' isn't present! Please query another field!\n")
		elif "where to watch"==field_name.lower():
